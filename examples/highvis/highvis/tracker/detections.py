...
class Detections:
    ...
    def __init__(self, frame_height: int, frame_width: int, prescaled_bbox=True) -> None:
        ...
        self.prescaled_bbox = prescaled_bbox
    ...
    def _is_SSD_IMX(self, result):
        ...
        if not isinstance(result, np.ndarray):
            logging.error("Result of wrong type.")
            return False
    ...
    def _load_SSD_IMX(self, result):
        ...
        # In some cases the bbox:es are not scaled to the input tensor dimension but instead of the full image.
        if self.prescaled_bbox:
            bbox = result[:, 0:4]
        else:
