...
class ScanSequence:
    ...
    def analyze(self, settings):
        ...
        result["T"] = self.trimmed_frames[0]["T"].isoformat()
    ...
    def time_diff(ts1, ts2):
        time_a = dateparser.parse(ts1)
        time_b = dateparser.parse(ts2)
    ...
    def elapsed_time(frames):
        ...
        logging.debug(f"Elapsed scan time: {elapsed_seconds.total_seconds()} seconds.")
        return elapsed_seconds.total_seconds()
    ...
    def class_is_background(mapped_class):
        if mapped_class == "BACKGROUND":
            return True
        elif mapped_class == "OBJECT" or mapped_class == "RESULT_OK" or mapped_class == "RESULT_BAD":
            return False
        else:
            return None
    ...
    def calculate_result(frames, settings):
        ...
        for frame in frames:
            if not (frame["mapped_class"] == "RESULT_OK" or frame["mapped_class"] == "RESULT_BAD"):
                continue
            P = int(frame["P"] * 100)
