...
class BaseState:
    ...
    def run(self, frame):
        if self.init_run:
            self.init_run = False
            self.reset_frames()
            self.start_time = frame["T"]
        class_label = self.fsm.mapper.class_to_label(frame["C"])
        mapped_class = self.fsm.mapper.map_label(class_label)
    ...
    def get_availability_time_threshold(self):
        ...
        return settings["Fsm"]["Production"]["TAKT_TIME"] * settings["Fsm"]["Production"]["AVAILABILITY_FACTOR"]
