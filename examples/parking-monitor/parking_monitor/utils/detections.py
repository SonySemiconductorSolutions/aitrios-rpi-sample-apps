...
class Detections:
    ...
    def update(self, metadata: dict):
        ...
        global last_boxes, last_scores, classes
        np_outputs = self.imx500.get_outputs(metadata=metadata, add_batch=True)
        if np_outputs is not None:
            boxes, last_scores, classes = postprocess_yolov8_detection(
                outputs=np_outputs,
                conf=self.BOX_MIN_CONFIDENCE,
                iou_thres=self.IOU_THRESHOLD,
                max_out_dets=self.MAX_OUT_DETS,
            )[0]
            last_boxes = scale_boxes(boxes, 1, 1, 640, 640, True, False)
        self.bbox = last_boxes
        self.confidence = last_scores
        self.class_id = classes
        self.tracker_id = self.format_track()
        return last_boxes, last_scores, classes
