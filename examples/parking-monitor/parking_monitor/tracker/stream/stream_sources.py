...
from parking_monitor.tracker.stream.utils import IMX500Receiver

HEADLESS = False
from importlib.metadata import distribution, PackageNotFoundError
import cv2
from parking_monitor.tracker.stream.stream_frame import Frame
...
class IMX500(AbstractSource):
    ...
    def imx5000_tensor_info_parser(self, tensor_info):
        network_name, *tensor_data_num, num_tensors = struct.unpack(
            "64s16II", bytes(tensor_info["Imx500OutputTensorInfo"])
        )
        network_name, self.width, self.height, num_channels = struct.unpack(
            "64sIII", bytes(tensor_info["Imx500InputTensorInfo"])
        )
